...
const ops = core.ops;
//[DEL] import {
  //[DEL] op_bootstrap_args,
  //[DEL] op_bootstrap_is_stderr_tty,
  //[DEL] op_bootstrap_is_stdout_tty,
  //[DEL] op_bootstrap_no_color,
  //op_bootstrap_pid,
  //op_main_module,
  //op_ppid,
  //[DEL] op_set_format_exception_callback,
  //op_snapshot_options,
  //[DEL] op_worker_close,
  //op_worker_get_type,
  //[DEL] op_worker_post_message,
  //[DEL] op_worker_recv_message,
  //[DEL] op_worker_sync_fetch,
//[DEL] } from "ext:core/ops";
const {
...
//[DEL] import { registerDeclarativeServer } from "ext:deno_http/00_serve.ts";
import * as event from "ext:deno_web/02_event.js";
import * as location from "ext:deno_web/12_location.js";
import * as version from "ext:custom_extension/01_version.js";
//[DEL] import * as os from "ext:runtime/30_os.js";
...
//[DEL] import * as fetch from "ext:deno_fetch/26_fetch.js";
...
[DEL]*/
import { errors } from "ext:custom_extension/01_errors.js";
import * as webidl from "ext:deno_webidl/00_webidl.js";
import { DOMException } from "ext:deno_web/01_dom_exception.js";
import {
  unstableForWindowOrWorkerGlobalScope,
  windowOrWorkerGlobalScope,
} from "ext:custom_extension/98_global_scope_shared.js";
import {
  mainRuntimeGlobalProperties,
  memoizeLazy,
} from "ext:custom_extension/98_global_scope_window.js";
/*[DEL]
import {
...
function postMessage(message, transferOrOptions = { __proto__: null }) {
  ...
  //[DEL] op_worker_post_message(data);
}
...
async function pollForMessages() {
  ...
  while (!isClosing) {
    const recvMessage = null;//[DEL] op_worker_recv_message();
...
function importScripts(...urls) {
  //[DEL] if (op_worker_get_type() === "module") {
  //[DEL]   throw new TypeError("Can't import scripts in a module worker.");
  //[DEL] }
  ...
  const scripts = []/*[DEL]op_worker_sync_fetch(
    parsedUrls,
...
const opArgs = ()=>[];//[DEL] memoizeLazy(() => op_bootstrap_args());
const opPid = ()=>0; //[DEL] memoizeLazy(() => op_bootstrap_pid());
setNoColorFns(
  () => false, //[DEL] op_bootstrap_no_color() || false, //[DEL] !op_bootstrap_is_stdout_tty(),
  () => false, //[DEL] op_bootstrap_no_color() || false, //[DEL] !op_bootstrap_is_stderr_tty(),
);
...
function runtimeStart(
  ...
  //[DEL] op_set_format_exception_callback(formatException);
  version.setVersions(
    denoVersion,
    v8Version,
    tsVersion,
...
ObjectDefineProperties(finalDenoNs, {
  pid: core.propGetterOnly(()=>0),//[DEL] core.propGetterOnly(opPid),
  // `ppid` should not be memoized.
  // https://github.com/denoland/deno/issues/23004
  ppid: core.propGetterOnly(()=>0),//[DEL] core.propGetterOnly(() => 0),//[DEL] op_ppid()),
  noColor: core.propGetterOnly(()=>false),//[DEL] core.propGetterOnly(() => false ),//[DEL] op_bootstrap_no_color()),
  args: core.propGetterOnly(()=>[]),//[DEL] core.propGetterOnly(opArgs),
  mainModule: core.propGetterOnly(()=>true),//[DEL] core.propGetterOnly(() => op_main_module()),
  ...
  exitCode: {
    get() {
      return 0 //[DEL] os.getExitCode();
    },
    set(value) {
      //[DEL] os.setExitCode(value);
    },
  },
});
...
const {
  ...
  v8Version,
  target,
} = {tsVersion:0, v8Version: 0, target: ""}//[DEL] op_snapshot_options();
...
function bootstrapMainRuntime(runtimeOptions, warmup = false) {
  if (!warmup) {
    ...
    if (mode === executionModes.run || mode === executionModes.serve) {
      ...
      core.addMainModuleHandler((main) => {
        if (ObjectHasOwn(main, "default")) {
          try {
            //[DEL] serve = registerDeclarativeServer(main.default);
    ...
    //[DEL] if (!ArrayPrototypeIncludes(unstableFeatures, unstableIds.unsafeProto)) {
      // Removes the `__proto__` for security reasons.
      // https://tc39.es/ecma262/#sec-get-object.prototype.__proto__
      delete Object.prototype.__proto__;
    //[DEL] }

    //[DEL] if (!ArrayPrototypeIncludes(unstableFeatures, unstableIds.temporal)) {
    ...
    //[DEL] ObjectDefineProperty(globalThis, "Deno", core.propReadOnly(finalDenoNs));
...
function bootstrapWorkerRuntime(
  ...
  if (!warmup) {
    ...
    //[DEL] ObjectDefineProperties(globalThis, workerRuntimeGlobalProperties);
    ObjectDefineProperties(globalThis, {
    ...
    /*[DEL] os.setExitHandler((_exitCode) => {
      workerClose();
    });[DEL]*/
    ...
    // TODO(bartlomieju): deprecate --unstable
    if (unstableFlag) {
      //[DEL] ObjectAssign(finalDenoNs, denoNsUnstable);
    } else {
      for (let i = 0; i <= unstableFeatures.length; i++) {
        //[DEL] const id = unstableFeatures[i];
        //[DEL] ObjectAssign(finalDenoNs, denoNsUnstableById[id]);
      }
    }
...
let _target = new event.EventTarget();
_target.dispatchEvent(new Event("warmup"));
